...
{
	initShader(L"normal_vs.cso", L"normal_gs.cso", L"normal_ps.cso");
}
...
void NormalShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* heightTexture, ID3D11ShaderResourceView* normalTexture, ID3D11ShaderResourceView* depthMap[], ID3D11ShaderResourceView* pointDepthMap[], LightA* light[])
{
	HRESULT result;
...
void NormalShader::initShader(const wchar_t* vs, const wchar_t* gs, const wchar_t* ps)
{
	initShader(vs, ps);
	loadGeometryShader(gs);
}

void NormalShader::initShader(const wchar_t* vs, const wchar_t* ps)
{
