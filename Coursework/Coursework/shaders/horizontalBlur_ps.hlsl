...
float4 main(InputType input) : SV_TARGET
{
    ...
    if (maskColour.x == 1.f)
    {
        ...
        colour += sceneTexture.Sample(Sampler0, input.tex + float2(texelSize * -4.0f, 0.0f)) * weight4;
        colour += sceneTexture.Sample(Sampler0, input.tex + float2(texelSize * -3.0f, 0.0f)) * weight3;
        colour += sceneTexture.Sample(Sampler0, input.tex + float2(texelSize * -2.0f, 0.0f)) * weight2;
        colour += sceneTexture.Sample(Sampler0, input.tex + float2(texelSize * -1.0f, 0.0f)) * weight1;
